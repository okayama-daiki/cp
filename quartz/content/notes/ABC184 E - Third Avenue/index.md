...
そこで，各アルファベットに対して隣接するような超頂点を作成し，アルファベットから超頂点へのコストが1，超頂点からアルファベットへのコストが0となるように辺を張る．
    ...
    if isinstance(uy, int):
        ...
        for dy, dx in ((0, 1), (0, -1), (1, 0), (-1, 0)):
            ...
            if (vy, vx) in alpha_nodes:
                # alphaの頂点からコスト0の辺を張る代わりに，
                # alphaに隣接する頂点からコスト1の辺を超頂点へ張る。
                # これらは等価である。
                dists_alpha[a[vy][vx]] = min(dists_alpha[a[vy][vx]], dists[uy][ux] + 1)
