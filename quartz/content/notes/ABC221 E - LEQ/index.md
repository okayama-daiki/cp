...
部分列の両端のインデックス$i,j$が決まれば，間の$j-i-1$個の要素に関しては取る／取らないの2通りの選択により部分列が構成されるため，この場合の部分列の総数は$2^{j-i-1}$個となる．
...
今，$\sum_{j\in\{i<j,A_i\le A_j\}}^{n}{2^{j-1}}$はBinary Indexed Treeにより高速に計算ができるので，$i$について全探索すれば解が得られる．
