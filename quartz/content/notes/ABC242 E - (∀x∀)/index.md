...
桁DPの考え方に近いが$S$に一致しているか否かが重要で，$S$より小さいのならば$A-Z$の好きな文字を割り当てることができる．そのため，例えば$S$の前半部分が`ABCD`であるなら，`ABCC`以下の文字では自由に回文を組むことができる．これは26進数であると考えれば容易に求まる．（繰り返し二乗法を利用しないとTLE地獄が発生するので注意）
