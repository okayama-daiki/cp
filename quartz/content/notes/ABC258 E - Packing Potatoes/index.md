...
- 1, 7と3, 5は共にじゃがいもの重さの和と個数が等しいが，中身が区別できない
...
そのため$S=\sum_{i=0}^{N-1}W_i$を1周期分として，$X$を$S$で割った余りに対して二分探索を行うことで高速に初期化を行うことができる．
...
# dob[p][i] := 2^p個目の箱に対して，
# i (mod N)番目のじゃがいもから詰めたときに，
# 次の箱に入れる最初のじゃがいもの番号 (mod N)
dob = [[0] * n for _ in range(41)]

# count[i] := i (mod N)番目のじゃがいもから詰めたとき，
# 蓋をするまでに入れるじゃがいもの個数
count = [0] * n
