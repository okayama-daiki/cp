...
$(a,b),(c,d),(e,f)$への移動を$N$回行うとすると，感覚的には$3^N$の程度の状態数を持ちそうだが，以下のコードがACしたことを考えると大体$N=300$でも$O(10^5)$個数程度であると分かる．
...
`collections.defaultdict`などを使うと生成時のオーバーヘッドによりTLEするので注意．
    ...
    for (x, y), v in dp[i].items():
        for dx, dy in (a, b), (c, d), (e, f):
            if (x + dx, y + dy) in block:
                continue
            dp[i + 1][x + dx, y + dy] = (dp[i + 1].get((x + dx, y + dy), 0) + v) % MOD
